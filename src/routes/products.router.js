    ...
    try {
      ...
      for (let i = startPage; i <= endPage; i++) {
        pagination.pages.push({
          page: i,
          link: `/api/products?page=${i}&limit=${limit}&sort=${sort || ''}&query=${query || ''}`,
          isCurrent: i === page
        });
      }
      ...
      if (startPage > 2) {
        pagination.pages.unshift({
          page: 1,
          link: `/api/products?page=1&limit=${limit}&sort=${sort || ''}&query=${query || ''}`
        });
      ...
      if (endPage < totalPages - 1) {
        ...
        pagination.pages.push({
          page: totalPages,
          link: `/api/products?page=${totalPages}&limit=${limit}&sort=${sort || ''}&query=${query || ''}`
        });
      }
